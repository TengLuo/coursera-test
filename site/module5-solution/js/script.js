...
function buildAndShowAboutHTML (aboutHtml) {
  switchAboutToActive();
  var starNum = randomRating();
  var finalHtml = aboutHtml;
  for (let i = 1; i <= 5; i++) {
    (starNum >= i) ? 
    finalHtml = insertProperty(finalHtml,i+"star","fa fa-star") : 
    finalHtml = insertProperty(finalHtml,i+"star","fa fa-star-o");
  }
  finalHtml = insertProperty(finalHtml,"starNum",starNum);
